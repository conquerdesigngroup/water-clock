...
A liquid-glass clock: the digital time feeds a Gray–Scott reaction–diffusion
system (liquid condenses out of the digits), a compact Navier–Stokes solver
makes it slosh and lets the pointer stir it, and a refraction shader renders
it as water over an analog background of three orbiting lens-circles.
It is a faithful from-scratch recreation of chiuhans111's "Fluid Glass"
(same algorithms and constants, none of the original code).
...
- `index.html` — markup shell shared by dev and dist (veil/hint/fail elements are referenced by JS — keep ids)
...
- `dist/water-clock.html` — generated; committed for double-click convenience. **Never edit by hand.**
...
2. `drawTimeMask` — rasterize HH:MM:SS to the 2D canvas (sim resolution), upload as `maskTex`; colon blinks each half-second; portrait stacks HH/MM/SS
...
5. copy `velTemp→vel`
6. `bgClock → bg` — three lens-circles orbit at continuous (ms-included) hand fractions; inter-layer refraction + parallax at depths ×2/×10/×20
...
URL params (all optional): `color`, `color1..3`, `bgcolor` (6-digit hex, `#` optional) · `shadow`, `bright`, `feed`, `kill` (floats) · `iteration` (int). Invalid values fall back to defaults.

Keyboard: `1`–`4` presets (ember / cosmos black / sticky pink / cozy blue) · `space` pause · `d` cycles debug views (off → thickness field → velocity → raw background).
